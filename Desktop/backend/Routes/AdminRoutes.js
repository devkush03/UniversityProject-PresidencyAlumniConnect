    ...
    try {
        ...
        if (id) {
            ...
            con.query(sql, [name, email, type, id], (err, result) => {
                ...
                if(hashedPassword){
                    const sql1 = 'UPDATE alumnus_bio SET name=?, email=? WHERE id=?';
                    con.query(sql1, [name, email, id], (err, result) => {
                        if (err) {
                            console.error('Error executing SQL query:', err);
                            return res.status(500).json({ error: 'Database Error' });
                        }
...
router.post("/upaccount", avatarUpload.single("avatar"), async (req, res) => {
    console.log("📩 /upaccount route hit");
    const {
      alumnus_id,
      user_id,
      course_id,
    ...
    try {
      ...
      const updateAlumnusQuery = `
        UPDATE alumnus_bio 
        SET name = ?, email = ?, course_id = ?, gender = ?, connected_to = ?, batch = ?
        WHERE id = ?
      `;
      await con.query(updateAlumnusQuery, [
        name,
        email,
        course_id,
        gender,
      ...
      console.log("✅ alumnus_bio updated");
      ...
      const updateUserQuery = `
        ...
        WHERE id = ?
      `;
      await con.query(updateUserQuery, [email, hashedPassword, user_id]);
      ...
      if (avatar) {
        const updateAvatarQuery = `
          ...
          WHERE id = ?
        `;
        const avatarPath = `Public/Avatar/${avatar}`; // Store relative path of avatar
        await con.query(updateAvatarQuery, [avatarPath, alumnus_id]);
...
router.get("/alumnusdetails", (req, res) => {
    ...
    const sql = "SELECT a.*, c.course FROM alumnus_bio a INNER JOIN courses c ON c.id = a.course_id WHERE a.id = ?";
    con.query(sql, [alumnusId], (err, result) => {
        if (err) return res.json({ Error: "Query Error" });
        if (result.length > 0) {
            return res.json(result);
        } else {
            return res.json({ message: "No Alumnus found with that ID" });
        }
    });
});
